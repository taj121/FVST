let process = fun x => x in 
 ...
 in (spawn coord); (loop ())) 
 in (spawn coord); 
 let clientInit = (fun z => let con = req-db () in 
 let f1 = (fun sql => (sel-QRY con); (send (con, sql)); (recv con) ) in 
 let f2 = (fun z => sel-END con ) in (f1, f2)) in 
